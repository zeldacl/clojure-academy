...
(defrecord ForgeSyncAdapter [sync-handler pos-adapter]
  ...
  (send-to-client [_ packet player]
    (let [network-manager (.connection player)]
      (.send network-manager 
             (.getPacketId MATRIX-CHANNEL)
  ...
  (encode-packet [_ {:keys [type data]} buf]
    (case type
      :matrix-state
      (doto buf
        (.writeVarInt 0)  ; packet type ID
        (.writeNbt (encode-state data)))
      
      :matrix-particles
      (let [{:keys [particle-type pos]} data]
        (doto buf
          (.writeVarInt 1)  ; packet type ID
          (.writeEnum particle-type)
          (.writeBlockPos (position/to-block-pos pos-adapter pos))))))
  
  (decode-packet [_ buf]
    (case (.readVarInt buf)
      0 {:type :matrix-state
         :data (decode-state (.readNbt buf))}
      1 {:type :matrix-particles
         :data {:particle-type (.readEnum buf)
