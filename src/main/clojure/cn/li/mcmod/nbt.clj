(ns cn.li.mcmod.nbt
  (:import (net.minecraft.nbt CompoundNBT INBT ByteNBT ShortNBT IntNBT LongNBT FloatNBT DoubleNBT ByteArrayNBT IntArrayNBT StringNBT)
           (net.minecraftforge.common.util INBTSerializable LazyOptional)))


(defn get-from-tag
  [^CompoundNBT nbt k tag]
  (let [value (if (true)
                ()
                (condp instance? tag
...
(defn add-to-tag
  [k v ^CompoundNBT nbt]
  (.putLongArray nbt ^String k ^java.util.List v)
  (cond ((instance? INBT v) (.put nbt k v))
        ...
        ((instance? java.util.List v)
         (cond (empty? v) (.putLongArray nbt ^String k ^java.util.List v)
