...
(defn imag-energy-item? [obj]
  (let [item (cond
               (instance? Item obj) obj
               (instance? ItemStack obj) (.getItem ^ItemStack obj)
               :else nil)]
...
(defn make-energy-transfer-stack-in-slot-fn
  ...
  ([rules]
   (fn [^Container container ^PlayerEntity player, ^long index]
     (let [^Slot slot (.getSlot container index)
           stack (if (.hasItem slot) (.getItem slot) ItemStack/EMPTY)
           ...
           merge (fn [rule]
                   (let [old-stack (.copy stack)
                         ...
                         to-slots (mapv #(.-index ^Slot %1) (filter #(to-slot-type? %1) (.-slots container)))
                         min (min to-slots)
                         max (max to-slots)]
                     (if (.moveItemStackTo ^Container container stack min max reverse-direction)
                       (do
                         (if (.isEmpty stack) (.set slot ItemStack/EMPTY) (.setChanged slot))
